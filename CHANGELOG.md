...
- **The reported action and the performed action disagreed.** A key retired
  from the source but still present in `dialect/translations/<locale>.arb` is
  an orphan in the generated output — and `--prune` printed
  `dropped 1 orphan key(s)` while removing nothing that mattered. The next
  `sync` regenerated the key straight back out of the translation, `check`
  reported the same `output_drift` forever, and the loop broke only by
  hand-editing the translation file. Reported from a Flutter project in
  `docs/feedback/2026-08-25-maisay-tooling-friction.md`.
- **`--prune` now deletes the orphan from `dialect/translations/` too**, which
  is what the word promises and what makes "pick one" true: `--adopt` already
  wrote to both canonical stores, and a `--prune` that resolved nothing could
  not be the other half of that choice.
- **Pruning happens before generation, not after it.** The old report was
  printed past the write loop, which is structurally why it could describe a
  deletion the run had not made.
- **The two populations are reported apart**, because lumping them together is
  what let the lie through: keys that live only in a generated file are
  *dropped by regenerating* and touch no canonical file; keys the translations
  still carry are *deleted*, named per locale, with the exact string quoted
  before anything is written. `git diff -- dialect/translations` is named as
  the undo.
- **`--prune --dry-run` prints that same list and writes nothing** — the way to
  read a data-loss decision before taking it, with no prompt an agent or CI
  cannot answer. It exits 1 on a pending deletion; it used to say
  `every output is up to date`, because with the orphan still in the
  translation the generated bytes really did match.
- **The refusal names the translation file as well as the output.** An orphan
  the translations still carry is not a stray line in a generated file, and
  someone who decides *not* to prune has to know where the second copy is.

### Added — the two papercuts that came with that report

- **`dialect` is stated to be an external CLI where people meet it.** The
  README's Install section and the AGENTS.md section `dialect init` writes into
  a project both say it is a standalone binary on PATH, not a package
  dependency, and that `dart run dialect` fails. In a Dart repo that is the
  natural first guess, and the correction used to live only at the README's
  top — which is not where a developer arriving mid-flow starts reading.
- **"Edit the ARB files by key, never by line range."** `check --fix` sorts
  every file by key, so a scripted *delete the text between key A and key B*
  takes whatever now sorts between them — it silently cost a live key in the
  field. Now in the README, in the AGENTS.md section, and in the translate
  plan's "Where to write". (It was caught in seconds only because `--prune`
  names every key it drops; that listing is deliberately unchanged.)
